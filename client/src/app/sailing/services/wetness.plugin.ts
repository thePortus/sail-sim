 ...
 *     up-facing surfaces.
 *   Phase 2 (this): standing deck PUDDLES (flat, up-facing deck floor above the waterline, pooled
 *     patchily where rain gathers) darkened toward wet wood, plus a wet SHEEN — an analytic
 *     sun-specular glint that is sharper on standing water and fades at night. The sheen replaces
 *     the roughness-drop the native shader does: Babylon exposes no injection point to lower
 *     `roughness` before its light loop, so instead we add our own highlight lobe at the proven
 *     CUSTOM_FRAGMENT_BEFORE_FRAGCOLOR point using the in-scope shading normal (normalW) and view
 *     vector (viewDirectionW). Still a flat waterline (the wave-following seaMap is a later phase).
 *   Later phases: wave-following waterline, puddle sky reflections + raindrop ripples.
 *
...
export class WetnessPlugin extends MaterialPluginBase {
  ...
  private static readonly GLSL = `
    if (wetEnabled > 0.5) {
      ...
      float wAbove  = step(wetSeaY, vPositionW.y) * (1.0 - step(wetSeaY + 4.0, vPositionW.y));
  ...
  private static readonly WGSL = `
    if (uniforms.wetEnabled > 0.5) {
      ...
      let wAbove  = step(uniforms.wetSeaY, fragmentInputs.vPositionW.y) * (1.0 - step(uniforms.wetSeaY + 4.0, fragmentInputs.vPositionW.y));
