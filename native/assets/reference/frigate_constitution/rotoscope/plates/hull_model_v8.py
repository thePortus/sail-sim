...
# smooth keel profile Zkeel(Y) aft of the forefoot (from the traced keel, faired)
keel=sorted([(p[0],p[1]) for p in W['keel']]+[tuple(W['sternpost'][0])])
kY=np.array([p[0] for p in keel]); kZ=np.array([p[1] for p in keel])
FOREFOOT_Y=-22.0
# faired cutwater Ycut(z): forefoot (z-6.30 -> Y-22.0) sweeping fwd/up to the stem head; smooth & monotone
YCUT=[(-6.30,-22.00),(-6.0,-22.35),(-5.5,-22.90),(-5.0,-23.35),(-4.5,-23.72),(-4.0,-24.05),
      (-3.5,-24.40),(-3.0,-24.72),(-2.5,-25.05),(-2.0,-25.38),(-1.5,-25.68),(-1.0,-25.98),
      (-0.5,-26.28),(0.0,-26.65),(0.5,-27.05),(1.0,-27.55),(1.5,-28.05),(2.0,-28.55),
      (2.5,-29.05),(3.0,-29.50),(3.5,-29.95),(4.0,-30.35),(4.7,-30.90)]
_cz=np.array([a[0] for a in YCUT]); _cy=np.array([a[1] for a in YCUT])
def Ycut(z): return float(np.interp(z,_cz,_cy))
def Zcut(Y): return float(np.interp(Y,_cy[::-1],_cz[::-1]))   # invert (Y decreasing with z)
def rabbet(Y):
    if Y<=FOREFOOT_Y: return Zcut(Y)               # forward of forefoot: follow the cutwater
    return float(np.interp(Y,kY,kZ))               # aft: smooth keel
...
fwd=[0.12,0.28,0.48,0.72]
sfwd=[section(v) for v in fwd]
stem_ring=[(0.0, round(Ycut(z),4), z) for (_,_,z) in sfwd[0]]
...
rings=[stem_ring]+sfwd+[section(s) for s in mid]+[counter, target, center]
