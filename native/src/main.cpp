...
static Clouds createClouds(WGPUDevice device, WGPUQueue queue, WGPUTextureFormat colorFormat) {
  ...
  // Premultiplied-alpha over the scene; depth-test (no write) so geometry occludes.
  WGPUBlendState blend = {};
  blend.color.srcFactor = WGPUBlendFactor_One; blend.color.dstFactor = WGPUBlendFactor_OneMinusSrcAlpha; blend.color.operation = WGPUBlendOperation_Add;
...
int main(int argc, char** argv) {
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    // Volumetric clouds — full-screen layer, premultiplied over the scene, depth-
    // tested (no write) so terrain/ships occlude it.
    wgpuRenderPassEncoderSetPipeline(pass, clouds.pipeline);
    wgpuRenderPassEncoderSetBindGroup(pass, 0, clouds.bindGroup, 0, nullptr);
    wgpuRenderPassEncoderDraw(pass, 3, 1, 0, 0);
