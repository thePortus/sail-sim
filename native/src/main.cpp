...
#include "ui_font.h"   // generated by CMake: embedded UI font (UI_FONT_TTF[])
...
int main(int argc, char** argv) {
  ...
  // Embedded JetBrains Mono, rasterised at the display's content scale for crisp
  // text on HiDPI; FontGlobalScale then brings it back to logical size.
  float uiScaleX = 1.0f, uiScaleY = 1.0f;
  glfwGetWindowContentScale(window, &uiScaleX, &uiScaleY);
  if (uiScaleX <= 0.0f) uiScaleX = 1.0f;
  ImFontConfig fontCfg;
  fontCfg.FontDataOwnedByAtlas = false;   // static array — don't let ImGui free it
  imio.Fonts->AddFontFromMemoryTTF((void*)UI_FONT_TTF, (int)UI_FONT_TTF_SIZE,
                                   18.0f * uiScaleX, &fontCfg);
