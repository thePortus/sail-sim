...
static PostFx createPostFx(WGPUDevice device, WGPUQueue queue, WGPUTextureFormat swapFormat) {
  ...
  // Post BGL: uniform + scene + bloom + sampler + ao + dof + depth + adapted-luminance (auto-exposure).
  {
    WGPUBindGroupLayoutEntry ble[8] = {};
    ...
    WGPUBindGroupLayoutDescriptor bgld = {}; bgld.entryCount = 8; bgld.entries = ble;
  ...
  WGPUBufferDescriptor ubd = {}; ubd.usage = WGPUBufferUsage_Uniform | WGPUBufferUsage_CopyDst;
  ubd.size = 7 * sizeof(glm::vec4); fx.postUbuf = wgpuDeviceCreateBuffer(device, &ubd);
...
static void rebuildPostBinds(WGPUDevice device, PostFx& fx, WGPUTextureView sceneView,
  ...
  release(fx.dofDownBind); release(fx.dofBlurBind);
  {
    ...
    be[0].binding = 0; be[0].buffer = fx.postUbuf; be[0].size = 7 * sizeof(glm::vec4);
    ...
    WGPUBindGroupDescriptor bgd = {}; bgd.layout = fx.postBGL; bgd.entryCount = 8; bgd.entries = be;
