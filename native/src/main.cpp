...
#include "ocean_wgsl.h"   // generated by CMake: inline const char* OCEAN_WGSL
...
// A Gerstner-wave ocean surface: a large flat grid displaced + shaded in WGSL.
struct Ocean {
  WGPURenderPipeline pipeline;
  WGPUBuffer         vbuf, ibuf, uniformBuf;
...
static Ocean createOcean(WGPUDevice device, WGPUQueue queue, WGPUTextureFormat colorFormat) {
  ...
  WGPUShaderModuleWGSLDescriptor wgsl = {};
  wgsl.chain.sType = WGPUSType_ShaderModuleWGSLDescriptor;
  wgsl.code = OCEAN_WGSL;
  WGPUShaderModuleDescriptor smd = {};
  smd.nextInChain = &wgsl.chain;
  o.module = wgpuDeviceCreateShaderModule(device, &smd);

  WGPUBindGroupLayoutEntry bgle = {};
  bgle.binding = 0; bgle.visibility = WGPUShaderStage_Vertex | WGPUShaderStage_Fragment;
  ...
  bge.binding = 0; bge.buffer = o.uniformBuf; bge.size = sizeof(OceanCamera);
  WGPUBindGroupDescriptor bgd = {};
  bgd.layout = bgl; bgd.entryCount = 1; bgd.entries = &bge;
  o.bindGroup = wgpuDeviceCreateBindGroup(device, &bgd);
...
int main(int argc, char** argv) {
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    // Ocean surface uniform.
    OceanCamera oc{ viewProj, glm::vec4(eye, 1.0f), glm::vec4(t, 0, 0, 0) };
    ...
    wgpuRenderPassEncoderSetBindGroup(pass, 0, ocean.bindGroup, 0, nullptr);
  ...
  wgpuBindGroupRelease(ocean.bindGroup);
