...
#include "taa_wgsl.h"            // generated by CMake: inline const char* TAA_WGSL (opt-in temporal AA)
#include "ssr_wgsl.h"            // generated by CMake: inline const char* SSR_WGSL (opt-in screen-space reflections)
#include "volumetric_wgsl.h"     // generated by CMake: inline const char* VOLUMETRIC_WGSL (opt-in volumetric sun shafts)
#include "lumadapt_wgsl.h"       // generated by CMake: inline const char* LUMADAPT_WGSL (opt-in auto-exposure metering)
#include "contact_wgsl.h"        // generated by CMake: inline const char* CONTACT_WGSL (opt-in screen-space contact shadows)
...
struct PostFx {
  ...
  // TAA (opt-in temporal AA): resolve pipeline + its own BGL/uniform. History textures and the
  // ping-pong bind groups are render-res, rebuilt on resize alongside the SMAA targets.
  WGPUShaderModule taaModule = nullptr;
  WGPURenderPipeline taaPipe = nullptr;
  WGPUBindGroupLayout taaBGL = nullptr;
  WGPUBuffer taaUbuf = nullptr;
  // SSR (opt-in screen-space reflections): reads the composited scene + depth, writes scene+reflection.
  WGPUShaderModule ssrModule = nullptr;
  WGPURenderPipeline ssrPipe = nullptr;
  WGPUBindGroupLayout ssrBGL = nullptr;
  WGPUBuffer ssrUbuf = nullptr;
  // Volumetric sun shafts (opt-in): marches the sun shadow map; additive blend into the HDR scene.
  WGPUShaderModule volModule = nullptr;
  WGPURenderPipeline volPipe = nullptr;
  WGPUBindGroupLayout volBGL = nullptr;
  WGPUBuffer volUbuf = nullptr;
  // Auto-exposure (opt-in): 1x1 luminance metering + eye adaptation feeding the post exposure.
  WGPUShaderModule lumModule = nullptr;
  WGPURenderPipeline lumPipe = nullptr;
  WGPUBindGroupLayout lumBGL = nullptr;
  WGPUBuffer lumUbuf = nullptr;
  WGPUTextureView lumHistView = nullptr;   // fixed 1x1 view the post pass samples (binding 7)
  // Contact shadows (opt-in): screen-space depth march toward the sun; multiply blend into the scene.
  WGPUShaderModule csModule = nullptr;
  WGPURenderPipeline csPipe = nullptr;
  WGPUBindGroupLayout csBGL = nullptr;
  WGPUBuffer csUbuf = nullptr;
  // 3D-LUT colour grade (opt-in): a 32^3 grade LUT sampled as the final post step.
  WGPUTextureView lutView = nullptr;       // fixed 3D view the post pass samples (binding 8)
...
static PostFx createPostFx(WGPUDevice device, WGPUQueue queue, WGPUTextureFormat swapFormat) {
  ...
  // Post BGL: uniform + scene + bloom + sampler + ao + dof + depth + adapted-luminance + 3D grade LUT.
  {
    WGPUBindGroupLayoutEntry ble[9] = {};
    ...
    ble[7].binding = 7; ble[7].visibility = WGPUShaderStage_Fragment;
    ble[7].texture.sampleType = WGPUTextureSampleType_UnfilterableFloat;
    ble[7].texture.viewDimension = WGPUTextureViewDimension_2D;
    ble[8].binding = 8; ble[8].visibility = WGPUShaderStage_Fragment;
    ble[8].texture.sampleType = WGPUTextureSampleType_Float;
    ble[8].texture.viewDimension = WGPUTextureViewDimension_3D;
    WGPUBindGroupLayoutDescriptor bgld = {}; bgld.entryCount = 9; bgld.entries = ble;
  ...
  // TAA BGL: uniform + current LDR + filtering sampler + scene depth (sampled via textureLoad) +
  ...
  // SSR BGL: uniform + scene colour (float) + filtering sampler + scene depth.
  fx.ssrModule = makeWGSL(device, SSR_WGSL);
  {
    WGPUBindGroupLayoutEntry ble[4] = {};
    ble[0].binding = 0; ble[0].visibility = WGPUShaderStage_Fragment; ble[0].buffer.type = WGPUBufferBindingType_Uniform;
    ble[1].binding = 1; ble[1].visibility = WGPUShaderStage_Fragment; ble[1].texture.sampleType = WGPUTextureSampleType_Float; ble[1].texture.viewDimension = WGPUTextureViewDimension_2D;
    ...
    ble[3].binding = 3; ble[3].visibility = WGPUShaderStage_Fragment; ble[3].texture.sampleType = WGPUTextureSampleType_Depth; ble[3].texture.viewDimension = WGPUTextureViewDimension_2D;
    WGPUBindGroupLayoutDescriptor bgld = {}; bgld.entryCount = 4; bgld.entries = ble;
    fx.ssrBGL = wgpuDeviceCreateBindGroupLayout(device, &bgld);
  }
  fx.ssrPipe = makePipe(fx.ssrModule, "fs_main", fx.ssrBGL, kSceneFormat, false);
  // Volumetric BGL: uniform + scene depth + shadow (cascade-0) depth + comparison sampler. The pipeline
  // uses ADDITIVE blend so it composites the shafts straight into the HDR scene (Load).
  fx.volModule = makeWGSL(device, VOLUMETRIC_WGSL);
  {
    WGPUBindGroupLayoutEntry ble[4] = {};
    ble[0].binding = 0; ble[0].visibility = WGPUShaderStage_Fragment; ble[0].buffer.type = WGPUBufferBindingType_Uniform;
    ble[1].binding = 1; ble[1].visibility = WGPUShaderStage_Fragment; ble[1].texture.sampleType = WGPUTextureSampleType_Depth; ble[1].texture.viewDimension = WGPUTextureViewDimension_2D;
    ble[2].binding = 2; ble[2].visibility = WGPUShaderStage_Fragment; ble[2].texture.sampleType = WGPUTextureSampleType_Depth; ble[2].texture.viewDimension = WGPUTextureViewDimension_2D;
    ble[3].binding = 3; ble[3].visibility = WGPUShaderStage_Fragment; ble[3].sampler.type = WGPUSamplerBindingType_Comparison;
    WGPUBindGroupLayoutDescriptor bgld = {}; bgld.entryCount = 4; bgld.entries = ble;
    fx.volBGL = wgpuDeviceCreateBindGroupLayout(device, &bgld);
  }
  {
    WGPUPipelineLayoutDescriptor pld = {}; pld.bindGroupLayoutCount = 1; pld.bindGroupLayouts = &fx.volBGL;
    WGPUPipelineLayout pl = wgpuDeviceCreatePipelineLayout(device, &pld);
    WGPUBlendState blend = {};
    ...
    WGPURenderPipelineDescriptor rpd = {};
    rpd.layout = pl;
    rpd.vertex.module = fx.volModule; rpd.vertex.entryPoint = "vs_main";
    rpd.primitive.topology = WGPUPrimitiveTopology_TriangleList; rpd.primitive.cullMode = WGPUCullMode_None;
    rpd.multisample.count = 1; rpd.multisample.mask = 0xFFFFFFFFu;
    rpd.fragment = &frag;
    fx.volPipe = wgpuDeviceCreateRenderPipeline(device, &rpd);
    wgpuPipelineLayoutRelease(pl);
  }
  // Auto-exposure metering BGL: uniform + scene (float 2D) + history (float 2D). Renders a 1x1
  // R16Float adapted-luminance value the post pass reads to self-level exposure.
  fx.lumModule = makeWGSL(device, LUMADAPT_WGSL);
  {
    WGPUBindGroupLayoutEntry ble[3] = {};
    ble[0].binding = 0; ble[0].visibility = WGPUShaderStage_Fragment; ble[0].buffer.type = WGPUBufferBindingType_Uniform;
    ble[1].binding = 1; ble[1].visibility = WGPUShaderStage_Fragment; ble[1].texture.sampleType = WGPUTextureSampleType_UnfilterableFloat; ble[1].texture.viewDimension = WGPUTextureViewDimension_2D;
    ble[2].binding = 2; ble[2].visibility = WGPUShaderStage_Fragment; ble[2].texture.sampleType = WGPUTextureSampleType_UnfilterableFloat; ble[2].texture.viewDimension = WGPUTextureViewDimension_2D;
    WGPUBindGroupLayoutDescriptor bgld = {}; bgld.entryCount = 3; bgld.entries = ble;
    fx.lumBGL = wgpuDeviceCreateBindGroupLayout(device, &bgld);
  }
  fx.lumPipe = makePipe(fx.lumModule, "fs_main", fx.lumBGL, WGPUTextureFormat_R16Float, false);
  // Contact-shadow BGL: uniform + scene depth. The pipeline uses a MULTIPLY blend (src*dst) so the
  // occlusion term darkens the HDR scene straight in place (Load).
  fx.csModule = makeWGSL(device, CONTACT_WGSL);
  {
    WGPUBindGroupLayoutEntry ble[2] = {};
    ble[0].binding = 0; ble[0].visibility = WGPUShaderStage_Fragment; ble[0].buffer.type = WGPUBufferBindingType_Uniform;
    ble[1].binding = 1; ble[1].visibility = WGPUShaderStage_Fragment; ble[1].texture.sampleType = WGPUTextureSampleType_Depth; ble[1].texture.viewDimension = WGPUTextureViewDimension_2D;
    WGPUBindGroupLayoutDescriptor bgld = {}; bgld.entryCount = 2; bgld.entries = ble;
    fx.csBGL = wgpuDeviceCreateBindGroupLayout(device, &bgld);
  }
  {
    WGPUPipelineLayoutDescriptor pld = {}; pld.bindGroupLayoutCount = 1; pld.bindGroupLayouts = &fx.csBGL;
    WGPUPipelineLayout pl = wgpuDeviceCreatePipelineLayout(device, &pld);
    WGPUBlendState blend = {};
    blend.color.operation = WGPUBlendOperation_Add; blend.color.srcFactor = WGPUBlendFactor_Dst; blend.color.dstFactor = WGPUBlendFactor_Zero;
    blend.alpha.operation = WGPUBlendOperation_Add; blend.alpha.srcFactor = WGPUBlendFactor_Zero; blend.alpha.dstFactor = WGPUBlendFactor_One;
    WGPUColorTargetState target = {}; target.format = kSceneFormat; target.blend = &blend; target.writeMask = WGPUColorWriteMask_All;
    WGPUFragmentState frag = {}; frag.module = fx.csModule; frag.entryPoint = "fs_main"; frag.targetCount = 1; frag.targets = &target;
    WGPURenderPipelineDescriptor rpd = {};
    rpd.layout = pl;
    rpd.vertex.module = fx.csModule; rpd.vertex.entryPoint = "vs_main";
    rpd.primitive.topology = WGPUPrimitiveTopology_TriangleList; rpd.primitive.cullMode = WGPUCullMode_None;
    rpd.multisample.count = 1; rpd.multisample.mask = 0xFFFFFFFFu;
    rpd.fragment = &frag;
    fx.csPipe = wgpuDeviceCreateRenderPipeline(device, &rpd);
    wgpuPipelineLayoutRelease(pl);
  }
  ...
  WGPUBufferDescriptor ubd = {}; ubd.usage = WGPUBufferUsage_Uniform | WGPUBufferUsage_CopyDst;
  ubd.size = 8 * sizeof(glm::vec4); fx.postUbuf = wgpuDeviceCreateBuffer(device, &ubd);
  ...
  ubd.size = 2 * sizeof(glm::mat4) + 4 * sizeof(glm::vec4);   // Volumetric: invVP + shadowVP + sunDir/sunCol/camPos/params
...
static void rebuildPostBinds(WGPUDevice device, PostFx& fx, WGPUTextureView sceneView,
  ...
  release(fx.dofDownBind); release(fx.dofBlurBind);
  {
    ...
    be[0].binding = 0; be[0].buffer = fx.postUbuf; be[0].size = 8 * sizeof(glm::vec4);
    ...
    be[7].binding = 7; be[7].textureView = fx.lumHistView;   // 1x1 adapted luminance (auto-exposure)
    be[8].binding = 8; be[8].textureView = fx.lutView;       // 3D grade LUT
    WGPUBindGroupDescriptor bgd = {}; bgd.layout = fx.postBGL; bgd.entryCount = 9; bgd.entries = be;
...
int main(int argc, char** argv) {
  ...
  if (std::getenv("SAILSIM_AUTOEXP")) userCfg.gfx.autoExposure = true;    // startup override for auto-exposure
  ...
  // SSR output (render res): the SSR pass writes scene+reflection here; when SSR is on, post reads
  // this instead of sceneView. (The SSR bind — which needs postFx — is built just below postFx.)
  WGPUTexture ssrTex = makeColorRTT(device, rW, rH, kSceneFormat);
  WGPUTextureView ssrView = wgpuTextureCreateView(ssrTex, nullptr);
  WGPUBindGroup ssrBind = nullptr;
  ...
  // ── Auto-exposure 1x1 luminance textures (fixed size, never resized). Each frame lumadapt reads the
  //    scene + the history, writes the newly-adapted value to lumDst, then we copy lumDst → lumHist so
  //    the post pass (binding 7 = lumHistView, fixed) samples this frame's value. History is seeded to
  //    a mid value so the first frames don't flash. ──
  auto makeLum1x1 = [&](WGPUTextureUsageFlags extra) {
    WGPUTextureDescriptor td = {};
    td.usage = WGPUTextureUsage_TextureBinding | extra;
    td.dimension = WGPUTextureDimension_2D; td.size = { 1, 1, 1 };
    td.format = WGPUTextureFormat_R16Float; td.mipLevelCount = 1; td.sampleCount = 1;
    return wgpuDeviceCreateTexture(device, &td);
  };
  WGPUTexture lumHistTex = makeLum1x1(WGPUTextureUsage_CopyDst | WGPUTextureUsage_RenderAttachment);
  WGPUTexture lumDstTex  = makeLum1x1(WGPUTextureUsage_RenderAttachment | WGPUTextureUsage_CopySrc);
  WGPUTextureView lumHistView = wgpuTextureCreateView(lumHistTex, nullptr);
  WGPUTextureView lumDstView  = wgpuTextureCreateView(lumDstTex, nullptr);
  ...
  {  // Seed the history to ~0.2 (mid daylight key) via a one-shot clear.
    ...
    WGPURenderPassEncoder rp = wgpuCommandEncoderBeginRenderPass(ce, &pd);
    wgpuRenderPassEncoderEnd(rp); wgpuRenderPassEncoderRelease(rp);
    WGPUCommandBuffer cb = wgpuCommandEncoderFinish(ce, nullptr);
    wgpuQueueSubmit(queue, 1, &cb);
    wgpuCommandBufferRelease(cb); wgpuCommandEncoderRelease(ce);
  }
  WGPUBindGroup lumBind = nullptr;
  auto rebuildLumBind = [&]() {
    if (lumBind) wgpuBindGroupRelease(lumBind);
    WGPUBindGroupEntry e[3] = {};
    e[0].binding = 0; e[0].buffer = postFx.lumUbuf; e[0].size = sizeof(glm::vec4);
    e[1].binding = 1; e[1].textureView = sceneView;
    e[2].binding = 2; e[2].textureView = lumHistView;
    WGPUBindGroupDescriptor d = {}; d.layout = postFx.lumBGL; d.entryCount = 3; d.entries = e;
    lumBind = wgpuDeviceCreateBindGroup(device, &d);
  };
  rebuildLumBind();
  // ── 3D colour-grade LUT (32^3): a subtle filmic grade baked once on the CPU — a gentle S-curve
  //    contrast, a shadows-toward-teal / highlights-toward-warm split-tone, and a small saturation
  //    lift. The post pass samples it (binding 8) as the final grade step, blended by amount. ──
  {
    const int N = 32;
    std::vector<uint8_t> lutData(N * N * N * 4);
    auto sat = [](float v) { return v < 0.0f ? 0.0f : (v > 1.0f ? 1.0f : v); };
    for (int zb = 0; zb < N; ++zb)
      for (int yg = 0; yg < N; ++yg)
        for (int xr = 0; xr < N; ++xr) {
          glm::vec3 c(xr / (float)(N - 1), yg / (float)(N - 1), zb / (float)(N - 1));
          // Gentle S-curve contrast (blended lightly so midtones stay honest).
          glm::vec3 sc = c * c * (3.0f - 2.0f * c);
          c = glm::mix(c, sc, 0.30f);
          float L = glm::dot(c, glm::vec3(0.2126f, 0.7152f, 0.0722f));
          float sh = 1.0f - glm::clamp(L / 0.5f, 0.0f, 1.0f);          // shadow weight
          float hi = glm::clamp((L - 0.5f) / 0.5f, 0.0f, 1.0f);        // highlight weight
          c += glm::vec3(-0.020f, 0.006f, 0.028f) * sh;               // shadows → teal
          c += glm::vec3(0.026f, 0.006f, -0.024f) * hi;               // highlights → warm
          c = glm::mix(glm::vec3(L), c, 1.10f);                        // small saturation lift
          int idx = ((zb * N + yg) * N + xr) * 4;
          lutData[idx + 0] = (uint8_t)(sat(c.r) * 255.0f + 0.5f);
          lutData[idx + 1] = (uint8_t)(sat(c.g) * 255.0f + 0.5f);
          lutData[idx + 2] = (uint8_t)(sat(c.b) * 255.0f + 0.5f);
          lutData[idx + 3] = 255;
        }
    WGPUTextureDescriptor td = {};
    td.usage = WGPUTextureUsage_TextureBinding | WGPUTextureUsage_CopyDst;
    td.dimension = WGPUTextureDimension_3D; td.size = { (uint32_t)N, (uint32_t)N, (uint32_t)N };
    td.format = WGPUTextureFormat_RGBA8Unorm; td.mipLevelCount = 1; td.sampleCount = 1;
    WGPUTexture lutTex = wgpuDeviceCreateTexture(device, &td);
    WGPUImageCopyTexture dst = {}; dst.texture = lutTex; dst.mipLevel = 0;
    WGPUTextureDataLayout dl = {}; dl.bytesPerRow = (uint32_t)(N * 4); dl.rowsPerImage = (uint32_t)N;
    WGPUExtent3D ext = { (uint32_t)N, (uint32_t)N, (uint32_t)N };
    wgpuQueueWriteTexture(queue, &dst, lutData.data(), lutData.size(), &dl, &ext);
    postFx.lutView = wgpuTextureCreateView(lutTex, nullptr);
  }
  auto rebuildSsrBind = [&]() {
    ...
    WGPUBindGroupDescriptor d = {}; d.layout = postFx.ssrBGL; d.entryCount = 4; d.entries = e;
  ...
  rebuildFxaaBind();
  // TAA history ping-pong (render-res kSceneFormat). taaBind[i] resolves INTO hist[i] while reading
  // hist[1-i] as the previous frame; taaBlitBind[i] passes hist[i] to the swapchain via fxaaPipe.
  WGPUTexture     taaHistTex[2]  = { nullptr, nullptr };
  WGPUTextureView taaHistView[2] = { nullptr, nullptr };
  WGPUBindGroup   taaBind[2]     = { nullptr, nullptr };
  WGPUBindGroup   taaBlitBind[2] = { nullptr, nullptr };
  auto rebuildTaaTargets = [&]() {
    for (int i = 0; i < 2; ++i) {
    ...
    for (int i = 0; i < 2; ++i) {
      ...
      b[0].binding = 0; b[0].buffer = postFx.fxaaUbuf; b[0].size = sizeof(glm::vec4);
      b[1].binding = 1; b[1].textureView = taaHistView[i];
      b[2].binding = 2; b[2].sampler = postFx.samp;
      WGPUBindGroupDescriptor bd = {}; bd.layout = postFx.fxaaBGL; bd.entryCount = 3; bd.entries = b;
      taaBlitBind[i] = wgpuDeviceCreateBindGroup(device, &bd);
    }
  };
  rebuildTaaTargets();
  ...
  float aimTubeFade = 0.0f;   // spikes to 1 on ANY shot, decays — dims ALL aim tubes while a broadside fires so the muzzle flashes/explosions read clean
...
@fragment fn fs(i: VO) -> @location(0) vec4<f32> {
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    if (w > 0 && h > 0 && (winChanged || scaleChanged)) {
      ...
      wgpuTextureViewRelease(ssrView); wgpuTextureRelease(ssrTex);
      ssrTex = makeColorRTT(device, rW, rH, kSceneFormat);
      ssrView = wgpuTextureCreateView(ssrTex, nullptr);
      rebuildSsrBind();
      rebuildLumBind();   // sceneView changed → re-point the auto-exposure metering
      rebuildFxaaBind();
      rebuildSmaaTargets();
      rebuildTaaTargets();
    ...
    if (appState == AppState::Sailing) {
      ...
      if (settingsOpen) {
        ...
        if (ImGui::Checkbox("Volumetric sun shafts", &g.volumetric)) { custom(); gsave(); }
        if (ImGui::IsItemHovered())
          ImGui::SetTooltip("Shadow-marched god-rays: real light beams through the rigging toward the sun.\nNeeds shadows on; adds a ray-march pass.");
        if (ImGui::Checkbox("Auto-exposure", &g.autoExposure)) { custom(); gsave(); }
        if (ImGui::IsItemHovered())
          ImGui::SetTooltip("Meter the scene and ease exposure toward a mid-grey key (eye adaptation),\nso bright decks and dim cabins self-level.");
        if (ImGui::Checkbox("Cinematic grade", &g.grade)) { custom(); gsave(); }
        if (ImGui::IsItemHovered())
          ImGui::SetTooltip("A gentle saturation push for a richer, filmic look.");
        if (ImGui::Checkbox("Colour-grade LUT", &g.lut)) { custom(); gsave(); }
        if (ImGui::IsItemHovered())
          ImGui::SetTooltip("Filmic 3D-LUT grade: teal shadows / warm highlights, gentle S-curve contrast.");
    ...
    if (appState == AppState::Sailing) {
      ...
      aimTubeFade = std::max(0.0f, aimTubeFade - dt * 1.1f);   // holds through a rolling broadside (re-spiked per shot), ~0.9s fade after the last gun
      bool firedThisFrame = false;
      for (const combat::FireEvent& fe : fires) {
        aimTubeFade = 1.0f;   // a broadside is firing → dim ALL aim tubes so the flashes/explosions read
      ...
      //    so water pools mid-deck and pours off the edges. Packed to R8 and uploaded to the deck map. ──
      {
        ...
        }
        {
          ...
          if (sprayBowCd <= 0.0f && shipSpeed > 0.8f &&
            ...
            sprayBowCd = 0.3f - 0.22f * spdN;   // near-continuous: ~3/s ghosting, ~12/s at full speed
    ...
    if (userCfg.gfx.taa) {
      ...
      taaJitterIdx++;
      float jx = halton(taaJitterIdx, 2u) - 0.5f;
      float jy = halton(taaJitterIdx, 3u) - 0.5f;
      proj[2][0] += (2.0f * jx) / (float)rW;
    ...
    // contrast surge, night grain, bloom threshold/weight, vignette.
    {
      ...
      // Cinematic grade: a gentle saturation push (0 = neutral) rides in zp.w; auto-exposure self-levels
      // the image toward a mid-grey key when enabled (ae.x). Both opt-in, off by default.
      const float gradeSat = (sailing && userCfg.gfx.grade) ? 0.14f : 0.0f;
      const float aeOn = (sailing && userCfg.gfx.autoExposure) ? 1.0f : 0.0f;
      const float lutOn = (sailing && userCfg.gfx.lut) ? 1.0f : 0.0f;
      glm::vec4 postU[8] = {
    ...
    //    after the ocean so the sea occludes low arcs correctly). ──
    {
      ...
      for (int s2 = 0; s2 < 2; ++s2) {
        ...
        const float tubeA = 1.0f - 0.88f * aimTubeFade;
    ...
    // ── Contact-shadow pass (opt-in): screen-space depth march toward the sun; multiply blend darkens
    ...
    if (userCfg.gfx.contactShadows && sailing && lightDir.y > 0.05f) {
      ...
      wgpuRenderPassEncoderSetPipeline(cp, postFx.csPipe);
      wgpuRenderPassEncoderSetBindGroup(cp, 0, cbind, 0, nullptr);
      wgpuRenderPassEncoderDraw(cp, 3, 1, 0, 0);
      wgpuRenderPassEncoderEnd(cp);
      wgpuRenderPassEncoderRelease(cp);
      wgpuBindGroupRelease(cbind);
    }

    // ── Volumetric sun-shaft pass (opt-in): march the view ray against the sun's tight (cascade-0)
    //    shadow map and accumulate Henyey-Greenstein in-scatter toward the sun, additive into the HDR
    //    scene. Gives true god-rays through the rigging near the ship. Needs the shadow map, so it
    //    gates on shadows being on + daylight; the bind group is built per-frame (cheap, feature-gated).
    if (userCfg.gfx.volumetric && sailing && lightDir.y > 0.08f && userCfg.gfx.shadows > 0) {
      struct VolUC { glm::mat4 invVP; glm::mat4 shadowVP; glm::vec4 sunDir; glm::vec4 sunCol; glm::vec4 camPos; glm::vec4 params; } vu;
      vu.invVP    = glm::inverse(viewProj);
      vu.shadowVP = volShadowVP;
      vu.sunDir   = glm::vec4(glm::normalize(sunDir), dayK);
      // Warm daylight tint; the accumulator sums stepLen over the march, so the density (params.y) is small.
      vu.sunCol   = glm::vec4(1.0f, 0.92f, 0.78f, 0.0f);
      vu.camPos   = glm::vec4(eye, 1.0f);
      vu.params   = glm::vec4(48.0f, 0.0011f, 600.0f, 0.76f);   // steps, density, max march dist, HG g
      wgpuQueueWriteBuffer(queue, postFx.volUbuf, 0, &vu, sizeof(vu));
      SunShadow& vsh = sunShadow(device);
      WGPUBindGroupEntry ve[4] = {};
      ve[0].binding = 0; ve[0].buffer = postFx.volUbuf; ve[0].size = 2 * sizeof(glm::mat4) + 4 * sizeof(glm::vec4);
      ve[1].binding = 1; ve[1].textureView = depthReadView;
      ve[2].binding = 2; ve[2].textureView = vsh.view0;
      ...
      wgpuRenderPassEncoderSetPipeline(vp2, postFx.volPipe);
      wgpuRenderPassEncoderSetBindGroup(vp2, 0, vbind, 0, nullptr);
      wgpuRenderPassEncoderDraw(vp2, 3, 1, 0, 0);
      wgpuRenderPassEncoderEnd(vp2);
      wgpuRenderPassEncoderRelease(vp2);
      wgpuBindGroupRelease(vbind);
    }
    ...
    if ((userCfg.gfx.ssr || userCfg.gfx.fog) && sailing) {
      ...
      ca.clearValue = WGPUColor{ 0.0, 0.0, 0.0, 1.0 };
      WGPURenderPassDescriptor pd = {}; pd.colorAttachmentCount = 1; pd.colorAttachments = &ca;
      WGPURenderPassEncoder sp = wgpuCommandEncoderBeginRenderPass(encoder, &pd);
      wgpuRenderPassEncoderSetPipeline(sp, postFx.ssrPipe);
      wgpuRenderPassEncoderSetBindGroup(sp, 0, ssrBind, 0, nullptr);
      wgpuRenderPassEncoderDraw(sp, 3, 1, 0, 0);
      wgpuRenderPassEncoderEnd(sp);
      wgpuRenderPassEncoderRelease(sp);
    }

    // ── Auto-exposure metering (opt-in): meter the composited scene into a 1x1 adapted-luminance
    //    value (eye adaptation), then copy it over the history so the post pass samples this frame's
    //    key. Runs before post; gated on gfx.autoExposure. ──
    if (userCfg.gfx.autoExposure && sailing) {
      ...
      ca.clearValue = WGPUColor{ 0.0, 0.0, 0.0, 1.0 };
      WGPURenderPassDescriptor pd = {}; pd.colorAttachmentCount = 1; pd.colorAttachments = &ca;
      WGPURenderPassEncoder lp2 = wgpuCommandEncoderBeginRenderPass(encoder, &pd);
      wgpuRenderPassEncoderSetPipeline(lp2, postFx.lumPipe);
      wgpuRenderPassEncoderSetBindGroup(lp2, 0, lumBind, 0, nullptr);
      wgpuRenderPassEncoderDraw(lp2, 3, 1, 0, 0);
      wgpuRenderPassEncoderEnd(lp2);
      wgpuRenderPassEncoderRelease(lp2);
      // Ping-pong: copy the freshly-adapted value over the history the post pass reads.
      WGPUImageCopyTexture src = {}; src.texture = lumDstTex;  src.mipLevel = 0;
      WGPUImageCopyTexture dst = {}; dst.texture = lumHistTex; dst.mipLevel = 0;
      WGPUExtent3D ext = { 1, 1, 1 };
      wgpuCommandEncoderCopyTextureToTexture(encoder, &src, &dst, &ext);
    }
    ...
    if (userCfg.gfx.taa) {
      ...
      wgpuQueueWriteBuffer(queue, postFx.taaUbuf, 0, &tu, sizeof(tu));
      {
        ...
        wgpuRenderPassEncoderSetPipeline(p, postFx.taaPipe);
        wgpuRenderPassEncoderSetBindGroup(p, 0, taaBind[taaCur], 0, nullptr);
      ...
      wgpuRenderPassEncoderEnd(fp);
      wgpuRenderPassEncoderRelease(fp);
      taaPrevVP = taaCurVP; taaHistValid = true; taaCur = 1 - taaCur;   // advance history
    } else if (userCfg.gfx.aa == 2) {
