...
struct PostFx {
  ...
  WGPUBindGroupLayout postBGL = nullptr, bloomBGL = nullptr;
...
static PostFx createPostFx(WGPUDevice device, WGPUTextureFormat swapFormat) {
  ...
  // Post BGL: uniform + scene + bloom + sampler.
  {
  ...
  fx.postPipe   = makePipe(fx.postModule, "fs_main", fx.postBGL, swapFormat, true);   // shares the UI pass (depth attached)
  ...
  WGPUBufferDescriptor ubd = {}; ubd.usage = WGPUBufferUsage_Uniform | WGPUBufferUsage_CopyDst;
  ubd.size = 3 * sizeof(glm::vec4); fx.postUbuf = wgpuDeviceCreateBuffer(device, &ubd);
...
// (Re)wire the post/bloom bind groups to the current scene/bloom targets (resize).
static void rebuildPostBinds(WGPUDevice device, PostFx& fx, WGPUTextureView sceneView,
  ...
  {
    WGPUBindGroupEntry be[4] = {};
    be[0].binding = 0; be[0].buffer = fx.postUbuf; be[0].size = 3 * sizeof(glm::vec4);
    ...
    WGPUBindGroupDescriptor bgd = {}; bgd.layout = fx.postBGL; bgd.entryCount = 4; bgd.entries = be;
...
int main(int argc, char** argv) {
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    // ── Bloom: brightpass into half-res A, blur A->B (H) then B->A (V). ──
    if (sailing) {
    ...
    //    bloom, vignette, grain, rain lens), then Dear ImGui on top. ──
    {
      WGPURenderPassColorAttachment ca = {};
