...
describe('CurrencyEntry Update Endpoints', () => {
  ...
  it('UPDATE /api/currency-entries/:id should update a single currency-entry from an owner', async () => {
    const res = await requestWithSupertest.put('/api/currency-entries/' + createdId.toString())
      .set('Authorization', `${ownerToken}`)
      .send({
        id: createdId,
        amount: 101
    ...
    expect(res.body).toHaveProperty('message', 'CurrencyEntry was updated successfully.');
  });

  it('UPDATE /api/currency-entries/:id should update a single currency-entry from an admin', async () => {
    const res = await requestWithSupertest.put('/api/currency-entries/' + createdId.toString())
  ...
  it('UPDATE /api/currency-entries/:id should request a request from an editor', async () => {
    const res = await requestWithSupertest.put('/api/currency-entries/' + createdId.toString())
  ...
  it('UPDATE /api/currency-entries/:id should reject request without proper authorization', async () => {
    const res = await requestWithSupertest.put('/api/currency-entries/' + createdId.toString())
      .send({
        id: createdId,
        amount: 101
  ...
  it('UPDATE /api/currency-entries/:id should reject request from regular users with no privileges', async () => {
    const res = await requestWithSupertest.put('/api/currency-entries/' + createdId.toString())
  ...
  it('UPDATE /api/currency-entries/:id should reject request for unknown item', async () => {
    const res = await requestWithSupertest.put('/api/currency-entries/9999')
      .set('Authorization', `${ownerToken}`)
      .send({
        id: '9999',
        amount: 100
    ...
    expect(res.body).toHaveProperty('message', 'Cannot update currencyEntry with id=9999. Maybe currencyEntry was not found!');
  });

  it('UPDATE /api/currency-entries should reject items with amounts that are not numbers', async () => {
    const res = await requestWithSupertest
      .put('/api/currency-entries/' + createdId.toString())
      .set('Authorization', `${ownerToken}`)
      .send({
        amount: 'not-a-number'
      });
  ...
  it('UPDATE /api/currency-entries should reject items with notes that are not strings', async () => {
    const res = await requestWithSupertest
      .put('/api/currency-entries/' + createdId.toString())
      .set('Authorization', `${ownerToken}`)
      .send({
  ...
  it('UPDATE /api/currency-entries/:id should log a change', async () => {
    await requestWithSupertest.put('/api/currency-entries/' + createdId.toString())
      .set('Authorization', `${ownerToken}`)
      .send({
        amount: 101
      });
    const changeRes = await requestWithSupertest
      .get(`/api/changes?page=0&size=5&table=CurrencyEntries&action=Update&itemId=${createdId}`)
      .set('Authorization', `${ownerToken}`);
