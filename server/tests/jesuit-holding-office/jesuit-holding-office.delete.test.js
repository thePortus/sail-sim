...
describe('JesuitHoldingOffice Endpoints', () => {
  ...
  it('DELETE /api/jesuits-holding-offices/:id should delete a single jesuitHoldingOffice by owner', async () => {
    const res = await requestWithSupertest.delete('/api/jesuits-holding-offices/' + createdId.toString())
      .set('Authorization', `${ownerToken}`);
    expect(res.status).toEqual(200);
    expect(res.type).toEqual(expect.stringContaining('json'));
    expect(res.body).toHaveProperty('message', 'JesuitHoldingOffice was deleted successfully!');
  });

  it('DELETE /api/jesuits-holding-offices/:id should delete a single jesuitHoldingOffice by admin', async () => {
    const res = await requestWithSupertest.delete('/api/jesuits-holding-offices/' + createdId.toString())
  ...
  it('DELETE /api/jesuits-holding-offices/:id should delete a single jesuitHoldingOffice by editor', async () => {
    const res = await requestWithSupertest.delete('/api/jesuits-holding-offices/' + createdId.toString())
  ...
  it('DELETE /api/jesuits-holding-offices/:id should reject deleting an unknown jesuitHoldingOffice', async () => {
    const res = await requestWithSupertest.delete('/api/jesuits-holding-offices/9999')
      .set('Authorization', `${ownerToken}`);
    expect(res.status).toEqual(500);
    expect(res.type).toEqual(expect.stringContaining('json'));
    expect(res.body).toHaveProperty('message', 'Cannot delete JesuitHoldingOffice with id=9999. Maybe JesuitHoldingOffice was not found!');
  });

  it('DELETE /api/jesuits-holding-offices/:id should reject request without proper authorization', async () => {
    const res = await requestWithSupertest.delete('/api/jesuits-holding-offices/' + createdId.toString());
    expect(res.status).toEqual(401);
  });

  it('DELETE /api/jesuits-holding-offices/:id should reject by regular users without privileges', async () => {
    const res = await requestWithSupertest.delete('/api/jesuits-holding-offices/' + createdId.toString())
  ...
  it('DELETE /api/jesuits-holding-offices/:id should log a change', async () => {
    await requestWithSupertest.delete('/api/jesuits-holding-offices/' + createdId.toString())
      .set('Authorization', `${ownerToken}`);
    const changeRes = await requestWithSupertest
      .get(`/api/changes?page=0&size=5&table=JesuitsHoldingOffices&action=Delete&itemId=${createdId}`)
      .set('Authorization', `${ownerToken}`);
