...
describe('JesuitStudyingArea Endpoints', () => {
  ...
  it('GET /api/jesuits-studying-areas should show all jesuits-studying-areas', async () => {
    const res = await requestWithSupertest.get('/api/jesuits-studying-areas');
    expect(res.status).toEqual(200);
    expect(res.type).toEqual(expect.stringContaining('json'));
    expect(res.body.length).toBeGreaterThanOrEqual(1);
  });

  it('GET /api/jesuits-studying-areas?page=1&size=5 should show paginated jesuits-studying-areas', async () => {
    const res = await requestWithSupertest.get('/api/jesuits-studying-areas?page=0&size=5');
    expect(res.status).toEqual(200);
    expect(res.type).toEqual(expect.stringContaining('json'));
    expect(res.body.rows.length).toBeGreaterThanOrEqual(1);
  });

  it('GET /api/jesuits-studying-areas?page=1&size=5&studyAreaId=9999 should filter items by studyAreaId', async () => {
    const res = await requestWithSupertest.get('/api/jesuits-studying-areas?page=0&size=5&studyAreaId=9999');
    expect(res.status).toEqual(200);
    expect(res.type).toEqual(expect.stringContaining('json'));
    expect(res.body.rows.length).toBe(0);
  });

  it('GET /api/jesuits-studying-areas?page=1&size=5&studyAreaId=9999 should search for items by studyAreaId', async () => {
    const res = await requestWithSupertest.get(`/api/jesuits-studying-areas?page=0&size=5&studyAreaId=${relatedIds.studyArea}`);
    expect(res.status).toEqual(200);
    expect(res.type).toEqual(expect.stringContaining('json'));
    expect(res.body.rows.length).toBeGreaterThanOrEqual(1);
  });

  it('GET /api/jesuits-studying-areas?page=1&size=5&firstCatalogId=9999 should filter items by firstCatalogId', async () => {
    const res = await requestWithSupertest.get('/api/jesuits-studying-areas?page=0&size=5&firstCatalogId=9999');
    expect(res.status).toEqual(200);
    expect(res.type).toEqual(expect.stringContaining('json'));
    expect(res.body.rows.length).toBe(0);
  });

  it('GET /api/jesuits-studying-areas?page=1&size=5&firstCatalogId=9999 should search for items by firstCatalogId', async () => {
    const res = await requestWithSupertest.get(`/api/jesuits-studying-areas?page=0&size=5&firstCatalogId=${relatedIds.firstCatalog}`);
    expect(res.status).toEqual(200);
    expect(res.type).toEqual(expect.stringContaining('json'));
    expect(res.body.rows.length).toBeGreaterThanOrEqual(1);
  });

  it('GET /api/jesuits-studying-areas/:id should return a single jesuitStudyingArea in full detail', async () => {
    const res = await requestWithSupertest.get('/api/jesuits-studying-areas/' + createdId.toString());
    expect(res.status).toEqual(200);
    expect(res.type).toEqual(expect.stringContaining('json'));
    ...
    expect(res.body).toHaveProperty('studyArea');
    expect(res.body).toHaveProperty('firstCatalog');
    expect(res.body.studyArea).toBeInstanceOf(Object);
    expect(res.body.firstCatalog).toBeInstanceOf(Object);
    
  });

  it('GET /api/jesuits-studying-areas/:id should give an error when looking for an unknown jesuitStudyingArea', async () => {
    const res = await requestWithSupertest.get('/api/jesuits-studying-areas/9999');
    expect(res.status).toEqual(500);
    expect(res.type).toEqual(expect.stringContaining('json'));
    expect(res.body).toHaveProperty('message', 'Cannot find JesuitStudyingArea with id=9999.');
  });
