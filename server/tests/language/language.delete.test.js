...
describe('Language Delete Endpoints', () => {
  ...
  it('DELETE /api/languages/:id should delete a single language by an owner', async () => {
    const res = await requestWithSupertest.delete('/api/languages/' + createdId.toString())
      .set('Authorization', `${ownerToken}`);
    expect(res.status).toEqual(200);
    expect(res.type).toEqual(expect.stringContaining('json'));
    expect(res.body).toHaveProperty('message', 'Language was deleted successfully!');
  });

  it('DELETE /api/languages/:id should delete a single language by an admin', async () => {
    const res = await requestWithSupertest.delete('/api/languages/' + createdId.toString())
  ...
  it('DELETE /api/languages/:id should reject a request by an editor', async () => {
    const res = await requestWithSupertest.delete('/api/languages/' + createdId.toString())
      .set('Authorization', `${editorToken}`);
    expect(res.status).toEqual(401);
  });

  it('DELETE /api/languages/:id should reject request without proper authorization', async () => {
    const res = await requestWithSupertest.delete('/api/languages/' + createdId.toString())
      .set('Authorization', `${editorToken}`);
    expect(res.status).toEqual(401);
  });

  it('DELETE /api/languages/:id should reject deleting an unknown language', async () => {
    const res = await requestWithSupertest.delete('/api/languages/9999')
      .set('Authorization', `${ownerToken}`);
    expect(res.status).toEqual(500);
    expect(res.type).toEqual(expect.stringContaining('json'));
    expect(res.body).toHaveProperty('message', 'Cannot delete language with id=9999. Maybe language was not found!');
  });
  ...
  it('DELETE /api/languages/:id should reject request from a regular user without elevated privileges', async () => {
    const res = await requestWithSupertest.delete('/api/languages/' + createdId.toString())
      .set('Authorization', `${userToken}`);
    expect(res.status).toEqual(401);
  });

  it('DELETE /api/languagess/:id should reject deleting a language referenced by other tables', async () => {
    ...
    expect(res.status).toEqual(500);
    expect(res.type).toEqual(expect.stringContaining('json'));
    expect(res.body).toHaveProperty('message', 'Cannot delete Language due to existing references in other tables');
  ...
  it('DELETE /api/languages/:id should log a change', async () => {
    await requestWithSupertest.delete('/api/languages/' + createdId.toString())
      .set('Authorization', `${ownerToken}`);
    const changeRes = await requestWithSupertest
      .get(`/api/changes?page=0&size=5&table=Languages&action=Delete&itemId=${createdId}`)
      .set('Authorization', `${ownerToken}`);
