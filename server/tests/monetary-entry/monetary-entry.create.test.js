...
describe('MonetaryEntry Create Endpoints', () => {
  ...
  it('CREATE /api/monetary-entries should accept valid data from an owner', async () => {
    const res = await requestWithSupertest
  ...
  it('CREATE /api/monetary-entries should accept valid data from an admin', async () => {
    const res = await requestWithSupertest
      .post('/api/monetary-entries')
  ...
  it('CREATE /api/monetary-entries should reject request from editor', async () => {
    const res = await requestWithSupertest
      .post('/api/monetary-entries')
  ...
  it('CREATE /api/monetary-entries should reject request without proper authorization', async () => {
    const res = await requestWithSupertest
      .post('/api/monetary-entries')
      .send({
        monetaryColumnId: relatedIds.monetaryColumn,
  ...
  it('CREATE /api/monetary-entries should reject request from regular users with no privileges', async () => {
    const res = await requestWithSupertest
      .post('/api/monetary-entries')
  ...
  it('CREATE /api/monetary-entries should reject items without thirdCatalogId', async () => {
    const res = await requestWithSupertest
      .post('/api/monetary-entries')
      .set('Authorization', `${ownerToken}`)
      .send({
    ...
    expect(res.body).toHaveProperty('message', ['Must contain a \'thirdCatalogId\' field!']);
  });

  it('CREATE /api/monetary-entries should reject items without monetaryColumnId', async () => {
    const res = await requestWithSupertest
      .post('/api/monetary-entries')
      .set('Authorization', `${ownerToken}`)
      .send({
    ...
    expect(res.status).toEqual(500);
    expect(res.body).toHaveProperty('message', ['Must contain a \'monetaryColumnId\' field!']);
  });

  it('CREATE /api/monetary-entries should reject items with thirdCatalogIds that are not numbers', async () => {
    const res = await requestWithSupertest
      .post('/api/monetary-entries')
      .set('Authorization', `${ownerToken}`)
      .send({
  ...
  it('CREATE /api/monetary-entries should reject items with monetaryColumnIds that are not numbers', async () => {
    const res = await requestWithSupertest
      .post('/api/monetary-entries')
      .set('Authorization', `${ownerToken}`)
      .send({
        monetaryColumnId: '123',
        thirdCatalogId: relatedIds.thirdCatalog,
        description: 'Test description'
      });
    expect(res.status).toEqual(500);
  ...
  it('CREATE /api/monetary-entries should reject items with descriptions that are not strings', async () => {
    const res = await requestWithSupertest
      .post('/api/monetary-entries')
      .set('Authorization', `${ownerToken}`)
      .send({
  ...
  it('CREATE /api/monetary-entries should reject items with notes that are not strings', async () => {
    const res = await requestWithSupertest
      .post('/api/monetary-entries')
      .set('Authorization', `${ownerToken}`)
      .send({
  ...
  it('CREATE /api/monetary-entries should log a change', async () => {
    ...
    const changeRes = await requestWithSupertest
      .get(`/api/changes?page=0&size=5&table=MonetaryEntries&action=Create&itemId=${itemRes.body.id}`)
      .set('Authorization', `${ownerToken}`);
