...
describe('MonetaryEntry Delete Endpoints', () => {
  ...
  it('DELETE /api/monetary-entries/:id should delete a single monetary-entry by an owner', async () => {
    const res = await requestWithSupertest.delete('/api/monetary-entries/' + createdId.toString())
      .set('Authorization', `${ownerToken}`);
    expect(res.status).toEqual(200);
    expect(res.type).toEqual(expect.stringContaining('json'));
    expect(res.body).toHaveProperty('message', 'MonetaryEntry was deleted successfully!');
  });

  it('DELETE /api/monetary-entries/:id should delete a single monetary-entry by an admin', async () => {
    const res = await requestWithSupertest.delete('/api/monetary-entries/' + createdId.toString())
  ...
  it('DELETE /api/monetary-entries/:id should reject a request by an editor', async () => {
    const res = await requestWithSupertest.delete('/api/monetary-entries/' + createdId.toString())
  ...
  it('DELETE /api/monetary-entries/:id should reject request without proper authorization', async () => {
    const res = await requestWithSupertest.delete('/api/monetary-entries/' + createdId.toString());
    expect(res.status).toEqual(401);
  });

  it('DELETE /api/monetary-entries/:id should reject deleting an unknown monetary-entry', async () => {
    const res = await requestWithSupertest.delete('/api/monetary-entries/9999')
      .set('Authorization', `${ownerToken}`);
    expect(res.status).toEqual(500);
    expect(res.type).toEqual(expect.stringContaining('json'));
    expect(res.body).toHaveProperty('message', 'Cannot delete monetaryEntry with id=9999. Maybe monetaryEntry was not found!');
  });
  ...
  it('DELETE /api/monetary-entries/:id should reject request from a regular user without elevated privileges', async () => {
    const res = await requestWithSupertest.delete('/api/monetary-entries/' + createdId.toString())
      .set('Authorization', `${userToken}`);
    expect(res.status).toEqual(401);
  });

  it('DELETE /api/monetary-entries/:id should log a change', async () => {
    await requestWithSupertest.delete('/api/monetary-entries/' + createdId.toString())
      .set('Authorization', `${ownerToken}`);
    const changeRes = await requestWithSupertest
      .get(`/api/changes?page=0&size=5&table=MonetaryEntries&action=Delete&itemId=${createdId}`)
      .set('Authorization', `${ownerToken}`);
