...
describe('Office Delete Endpoints', () => {
  ...
  it('DELETE /api/offices/:id should delete a single office by an owner', async () => {
    const res = await requestWithSupertest.delete('/api/offices/' + createdId.toString())
      .set('Authorization', `${ownerToken}`);
    expect(res.status).toEqual(200);
    expect(res.type).toEqual(expect.stringContaining('json'));
    expect(res.body).toHaveProperty('message', 'Office was deleted successfully!');
  });

  it('DELETE /api/offices/:id should delete a single office by an admin', async () => {
    const res = await requestWithSupertest.delete('/api/offices/' + createdId.toString())
  ...
  it('DELETE /api/offices/:id should reject a request by an editor', async () => {
    const res = await requestWithSupertest.delete('/api/offices/' + createdId.toString())
      .set('Authorization', `${editorToken}`);
    expect(res.status).toEqual(401);
  });

  it('DELETE /api/offices/:id should reject request without proper authorization', async () => {
    const res = await requestWithSupertest.delete('/api/offices/' + createdId.toString())
      .set('Authorization', `${editorToken}`);
    expect(res.status).toEqual(401);
  });

  it('DELETE /api/offices/:id should reject deleting an unknown office', async () => {
    const res = await requestWithSupertest.delete('/api/offices/9999')
      .set('Authorization', `${ownerToken}`);
    expect(res.status).toEqual(500);
    expect(res.type).toEqual(expect.stringContaining('json'));
    expect(res.body).toHaveProperty('message', 'Cannot delete office with id=9999. Maybe office was not found!');
  });
  ...
  it('DELETE /api/offices/:id should reject request from a regular user without elevated privileges', async () => {
    const res = await requestWithSupertest.delete('/api/offices/' + createdId.toString())
      .set('Authorization', `${userToken}`);
    expect(res.status).toEqual(401);
  });

  it('DELETE /api/offices/:id should reject deleting a office referenced by other tables', async () => {
    ...
    expect(res.status).toEqual(500);
    expect(res.type).toEqual(expect.stringContaining('json'));
    expect(res.body).toHaveProperty('message', 'Cannot delete Office due to existing references in other tables');
    await requestWithSupertest.delete(`/api/jesuits-holding-offices/${jesuitHoldingOfficeResponse.body.id}`)
      .set('Authorization', `${ownerToken}`);
  ...
  it('DELETE /api/offices/:id should log a change', async () => {
    await requestWithSupertest.delete('/api/offices/' + createdId.toString())
      .set('Authorization', `${ownerToken}`);
    const changeRes = await requestWithSupertest
      .get(`/api/changes?page=0&size=5&table=Offices&action=Delete&itemId=${createdId}`)
      .set('Authorization', `${ownerToken}`);
