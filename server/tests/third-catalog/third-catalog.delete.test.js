...
describe('ThirdCatalog Delete Endpoints', () => {
  ...
  it('DELETE /api/third-catalogs/:id should delete a single third-catalog by an owner', async () => {
    const res = await requestWithSupertest.delete('/api/third-catalogs/' + createdId.toString())
      .set('Authorization', `${ownerToken}`);
    expect(res.status).toEqual(200);
    expect(res.type).toEqual(expect.stringContaining('json'));
    expect(res.body).toHaveProperty('message', 'ThirdCatalog was deleted successfully!');
  });

  it('DELETE /api/third-catalogs/:id should delete a single third-catalog by an admin', async () => {
    const res = await requestWithSupertest.delete('/api/third-catalogs/' + createdId.toString())
  ...
  it('DELETE /api/third-catalogs/:id should reject a request by an editor', async () => {
    const res = await requestWithSupertest.delete('/api/third-catalogs/' + createdId.toString())
  ...
  it('DELETE /api/third-catalogs/:id should reject request without proper authorization', async () => {
    const res = await requestWithSupertest.delete('/api/third-catalogs/' + createdId.toString());
    expect(res.status).toEqual(401);
  });

  it('DELETE /api/third-catalogs/:id should reject deleting an unknown third-catalog', async () => {
    const res = await requestWithSupertest.delete('/api/third-catalogs/9999')
      .set('Authorization', `${ownerToken}`);
    expect(res.status).toEqual(500);
    expect(res.type).toEqual(expect.stringContaining('json'));
    expect(res.body).toHaveProperty('message', 'Cannot delete thirdCatalog with id=9999. Maybe thirdCatalog was not found!');
  });
  ...
  it('DELETE /api/third-catalogs/:id should reject request from a regular user without elevated privileges', async () => {
    const res = await requestWithSupertest.delete('/api/third-catalogs/' + createdId.toString())
      .set('Authorization', `${userToken}`);
    expect(res.status).toEqual(401);
  });

  it('DELETE /api/third-catalogs/:id should log a change', async () => {
    await requestWithSupertest.delete('/api/third-catalogs/' + createdId.toString())
      .set('Authorization', `${ownerToken}`);
    const changeRes = await requestWithSupertest
      .get(`/api/changes?page=0&size=5&table=ThirdCatalogs&action=Delete&itemId=${createdId}`)
      .set('Authorization', `${ownerToken}`);
